...
public class PageInterceptor implements Interceptor {
			...
			if (selectId.substring(selectId.lastIndexOf(".") + 1).toLowerCase().endsWith(SELECT_ID_SUFFIX)) {
				...
				try {
					...
					if (paging) {
						...
						if (totalCount < 0) {
							// 重写sql
							String countSql = concatCountSql(sqlOriginal, sqlLowercase);
						...
						pageSql = concatPageSql(sqlOriginal, sqlLowercase, totalCount, offset, pageSize, sortAscend,
								sortColumn);
	 ...
	 * @return
	 */
	private String concatCountSql(String sqlOriginal, String sqlLowercase) {
	 ...
	 * @return
	 */
	private String concatPageSql(String sqlOriginal, String sqlLowercase, int totalCount, int offset, int pageSize,
			boolean sortAscend, String sortColumn) {
		int union = PageHelper.firstKeyword(sqlLowercase, PageHelper.UNION, 0);
		int group = PageHelper.firstKeyword(sqlLowercase, PageHelper.GROUP, 0);
		int distinct = PageHelper.firstKeyword(sqlLowercase, PageHelper.DISTINCT, 0);
		int firstParameter = sqlLowercase.indexOf("?");
		...
		if (subquery) {
			sb.insert(0, String.format("SELECT *,%d totalCount  FROM (", totalCount));
			// 如果最后的字符是分号,去掉
			...
			// 第一个select后面添加总条数字段
			sb.insert(sqlLowercase.indexOf("select") + 6, String.format(" %d totalCount ,", totalCount));
			// 如果最后的字符是分号,去掉
			if (sb.charAt(sb.length() - 1) == ';') {
				sb.deleteCharAt(sb.length() - 1);
			}
		...
		if (StringUtils.hasText(sortColumn)) {
			...
			String sortSnakeColumn = PageHelper.snakeName(sortColumn);
			...
			PageWord wordForSnake = PageHelper.firstColumn(sqlOriginal, sortSnakeColumn, 0);
			PageWord word = PageHelper.firstColumn(sqlOriginal, sortColumn, 0);
			if (wordForHump != null && wordForHump.getBegin() > 0 && wordForHump.getBegin() < topFrom) {
				if (subquery) {
					sb.append(" ORDER BY ").append(sortHumpColumn);
				} else {
					sb.append(" ORDER BY ")
							.append(sqlOriginal.subSequence(wordForHump.getBegin(), wordForHump.getBegin() + wordForHump.getLength()));
				}
				if (sortAscend) {
					sb.append(" ASC");
				} else {
			...
			} else if(wordForSnake != null && wordForSnake.getBegin() > 0 && wordForSnake.getBegin() < topFrom){

				if (subquery) {
					sb.append(" ORDER BY ").append(sortSnakeColumn);
				} else {
					sb.append(" ORDER BY ")
							.append(sqlOriginal.subSequence(wordForSnake.getBegin(), wordForSnake.getBegin() + wordForSnake.getLength()));
				}
				if (sortAscend) {
					sb.append(" ASC");
				} else {
			...
			}else if (word != null && word.getBegin() < topFrom) {
				if (subquery) {
