...
(function ($) {
	$.fn.extend({
				...
				$.each(opts.columns,function(j,columnitem){
					...
					//如果是无类型数据，按文本输出
						...
						//按键
							...
							//var button=
							var tablebtn = "<a href='javascript:void(0)' class='table_button' data-index-row='" + i + "' data-index-col='"+j+"'>"+columnitem.button.text+"</a>";
						...
						else if(columnitem.type=="imagebutton"){
							var imgbtnlist=[],stylelist=[];
							if(!currdiv.isNullObject(columnitem.list)){
								$.each(columnitem.list,function(i1,listitem){
									if(listitem.visible){
										imgbtnlist.push("<a href='javascript:void(0)' class='imagebutton imgbtn_"+listitem.command+"' data-cmd='"+listitem.command+"' style='width:"+listitem.width+"px;' title='"+listitem.describe+"' data-index-row='" + i + "' data-index-col='"+j+"'></a>");
										stylelist.push(".imgbtn_"+listitem.command+"{background-image:url("+listitem.url+");}");
										if(!currdiv.isNullObject(listitem.hoverurl))
											stylelist.push(".imgbtn_"+listitem.command+":hover{background-image:url("+listitem.hoverurl+");}");
									}
								})
							}
							if(stylelist.length>0){
								$("head").append("<style>"+stylelist.join('')+"</style>")
							}
							createTempTd=("<td colnum='"+j+"' class='"+classname+"'><div col='"+j+"' class='col colimagebutton' "+tablestyle+">"+imgbtnlist.join('')+"</div></td>");
						}
					}
		...
		"showSumNumber":function(){
			...
			showsum.unbind("hover").hover(function(){
				...
				$.each(opts.data,function(i,item){
					...
					if(itemvalue!=null)
						currsumnumber+=parseFloat(itemvalue);
				});
				prevnum=indexcolnum;
				hasshowtips=true;
				currdiv.layersMoretips((currhoverevent.eq(0).text() + "：（总）" + currdiv.formartDecimal_f(currsumnumber==false?0.0:currsumnumber)), currhoverevent.eq(0));
			}, function () {
		...
		"imagebutton":function(){
			var currdiv=this;
			var opts=this.opts;
			currdiv.find(".imagebutton").click(function(){
				var currbtn=$(this);
				var indexrownum=parseInt(currbtn.attr("data-index-row"));
				var indexcolnum=parseInt(currbtn.attr("data-index-col"));
				var curroptdata=opts.data[indexrownum];
				if(opts.imagebutton!=undefined)
					opts.imagebutton(indexrownum,indexcolnum,curroptdata,currbtn.attr("data-cmd"));
			}).hover(function(){
				//var currbtn=$(this);
				//layer.tips(currbtn.attr("alt"), currbtn);
			},function(){});
		}
	});
