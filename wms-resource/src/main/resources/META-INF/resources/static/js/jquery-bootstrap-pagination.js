  ...
  (function($) {
    ...
    return PaginationView = (function() {
      function PaginationView(el, options) {
        ...
        this.shows = __bind(this.shows, this);
        defaults = {
          current_page: 1,
          total_pages: 1,
          next: "&gt;",
        ...
        if ($(document).on) {
          $(this.el).on("click", "span", this.goto);
        } else {
          $("span", this.el).live("click", this.goto);
        }
      ...
      PaginationView.prototype.render = function() {
        var html, link, _i, _len, _ref,showPage;
        this.el.html("");
        ...
        html.push("</ul>");
        html.push("<div class='pagination form-inline'>到第 <input type='text' class='form-control GotoPage'> 页 ")
        html.push("<span class='btn btn-default'>确定</span></div>")
        if (this.settings.is_number) {
          html.push("　每页显示 <select class='form-control showPage' onchange='showPage()'><option value='20'>20</option><option value='50'>50</option><option value='100'>100</option><option value='500'>500</option><option value='1000'>1000</option></select> 条")
        }
        html.push("</div>");
        ...
        if (this.settings.is_number) {
          $("select.showPage").val($.cookie("pageSize"+GetURLKey())||20)
        }
      ...
      //跳转
      PaginationView.prototype.goto = function(event) {
        var page;

        page = parseInt($(".GotoPage").val());
        if (!this.isValidPage(page)) {
          return;
        }
        if (this.settings.callback != null) {
